...
<div class="container-fluid flex-grow-1 container-p-y">
    <div class="row">
        <div class="col-md-6 text-start">
            <h5 class="py-2 mb-2">
                <span class="text-primary fw-light">Sub Company</span>
            </h5>
        </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Sub Company Add</h5>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="hidden" id="code" value="cat" class="form-control" placeholder="Enter Code" />
                        <small class="error-text text-danger"></small>
                    </div>
                </div>
            </div>
...
<div class="modal fade" id="editModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Sub Company Edit</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        {{--  <label for="code" class="form-label">Code</label>  --}}
                        <input type="hidden" id="editcode" class="form-control" placeholder="" />
                        <small class="error-text text-danger"></small>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        const table = $('#variationTable').DataTable({
            processing: true,
            ajax: {
                url: "{{ route('company.subcompany.getall') }}",
                type: 'GET',
            },
            columns: [
                { data: "name" },
        ...
        $('#AddVariation').click(function(e) {
            ...
            $.ajax({
                url: '{{ route('company.subcompany.store') }}',
                type: 'POST',
                data: data,
        ...
        window.editUser = function(userId) {
            const url = '{{ route("company.subcompany.get", ":userid") }}'.replace(":userid", userId);
            $.ajax({
                url: url,
                method: 'GET',
                success: function(data) {
                    ...
                    setFlash("success", 'Sub Company found successfully.');
                },
                error: function() {
                    setFlash("error", "Sub Company not found. Please try again later.");
                }
            });
        };
        ...
        $('#EditUser').on('click', function() {
            ...
            $.ajax({
                url: '{{ route('company.subcompany.update') }}',
                method: 'POST',
                data: {
                    _token: $('meta[name="csrf-token"]').attr('content'),
        ...
        window.updateUserStatus = function(userId, status) {
            const message = status === "active" ? "Sub Company will be able to log in after activation." : "Sub Company will not be able to log in after deactivation.";
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('company.subcompany.status') }}",
                        data: { userId, status, _token: $('meta[name="csrf-token"]').attr('content') },
                        success: function (response) {
                            if (response.success) {
                                const successMessage = status === "active" ? "Sub Company activated successfully." : "Sub Company deactivated successfully.";
        ...
        window.deleteUser = function(userId) {
            Swal.fire({
                title: "Are you sure?",
                text: "Do you want to delete this Sub Company?",
                icon: "warning",
                showCancelButton: true,
                confirmButtonColor: "#3085d6",
                cancelButtonColor: "#d33",
                ...
                if (result.isConfirmed) {
                    const url = '{{ route("company.subcompany.destroy", ":userId") }}'.replace(":userId", userId);
                    $.ajax({
                        type: "DELETE",
                        url,
                        ...
                        success: function (response) {
                            if (response.success) {
                                setFlash("success", "Sub Company deleted successfully.");
                            } else {
                                setFlash("error", "There was an issue deleting the subcompany. Please contact your system administrator.");
                            }
                            table.ajax.reload();
                        },
                        error: function () {
